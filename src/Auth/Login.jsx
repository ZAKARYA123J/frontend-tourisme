...
const Login = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-sky-400 to-indigo-300 px-4 overflow-hidden">
          ...
          {isSignUp ? (
            <>
              ...
              <form onSubmit={handleSubmitRegister} className="space-y-5">
                ...
                <button type="submit" className="w-full py-3 bg-sky-600 hover:bg-sky-700 text-white font-medium rounded-lg transition-all duration-700">{loading ? "Inscription..." : "Register"}</button>
              ...
              <p className="text-center mt-4 text-sm">Already have an account? <span onClick={() => setIsSignUp(false)} className="text-sky-600 cursor-pointer hover:text-sky-800 transition-all duration-500">Sign In</span></p>
            </>
          ) : (
            <>
              ...
              <form onSubmit={handleSubmitLogin} className="space-y-5">
                ...
                <button type="submit" className="w-full py-3 bg-sky-600 hover:bg-sky-700 text-white font-medium rounded-lg transition-all duration-700" disabled={loading}>{loading ? "Connexion..." : "Sign In"}</button>
                {error && <div className="mt-3 text-red-600 text-sm text-center">{error}</div>}
                {success && <div className="mt-3 text-green-600 text-sm text-center">{success}</div>}
              </form>
              <p className="text-center mt-4 text-sm">Don't have an account? <span onClick={() => setIsSignUp(true)} className="text-sky-600 cursor-pointer hover:text-sky-800 transition-all duration-500">Register</span></p>
              <div className="text-center mt-4 text-sm">
                <Link to="/forgot-password" className="text-sky-600 cursor-pointer hover:text-sky-800 transition-all duration-500">Forgot Password?</Link>
              </div>
            </>
          )}
        </div>
        <div className={`w-1/2 bg-gradient-to-br from-indigo-400 to-sky-500 text-white flex flex-col justify-center items-center rounded-r-2xl p-10 transition-all duration-1000 ease-in-out ${isSignUp ? 'order-1' : 'order-2'}`}>
          ...
          <button onClick={() => setIsSignUp(!isSignUp)} className="mt-6 py-2 px-6 border border-white rounded-full text-white hover:bg-white hover:text-indigo-600 transition-all duration-700">
