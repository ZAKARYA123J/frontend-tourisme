...
const NavBar = () => {
  ...
  return (
    <div className={`flex w-full justify-between items-center h-20 px-6 fixed top-0 left-0 z-50 transition-all duration-300 ${isLoginPage ? "bg-gray-200 text-slate-800" : isScrolled ? "bg-white/90 backdrop-blur-sm text-slate-800 shadow-md" : "bg-transparent text-white"}`}>
            ...
            <li key={section.title} className="group relative py-2">
              ...
              <div className="absolute top-full left-0 hidden group-hover:block w-48 bg-white text-gray-700 shadow-lg rounded-lg py-2">
                {section.items.map((item, index) => {
                  ...
                  const itemLink = typeof item === "string" ? `${section.link}/${item.toLowerCase().replace(/\s+/g, '-')}` : item.link;
                  return (
                    <Link
      ...
      <div className="hidden md:flex items-center space-x-4">
        {user ? (
          <div className="relative">
            <div className="flex items-center space-x-3 cursor-pointer bg-white text-gray-700 px-3 py-2 rounded-full shadow-md hover:shadow-lg transition duration-300" onClick={toggleMenu}>
